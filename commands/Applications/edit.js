...
module.exports = {
  ...
  async execute(client, message, cmd, args) {
    ...
    async function editApplication({
      ...
      db.ref(`Applications/${guildID}/Positions${number.toString()}`).set(
        object
      );
      ...
      collector.on("collect", async (msg) => {
        ...
        if (step == 1) {
          ...
          message.channel.send(
            "What should be the new description of the application? If you want to use the existing one specify `none`"
          );
        } else if (step == 2) {
          ...
          message.channel.send(
            "What questions do you want in the application? Say `done` when you have put all the questions. If you want to use existing use `none`"
          );
        } else if (step >= 4) {
          if (msg.content.toLowerCase() == "none") {
