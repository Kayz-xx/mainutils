  ...
  async execute(client, message, cmd, args) {
    ...
    async function addApplication({
      ...
      if (typeof guildID !== "string")
        throw new Error("guildID must be a string");
      if (!Array.isArray(questions))
        throw new Error("questions must be an array");
      if (typeof name !== "string") throw new Error("name must be a string");
      if (typeof description !== "string")
        throw new Error("description must be a string");
      if (typeof responseChannelID !== "string")
        throw new Error("responseChannelID must be a string");
    ...
    async function setup(message) {
      ...
      const filter = (m) =>
        m.author.id === (message.author?.id || message.user?.id);
      ...
      collector.on("collect", async (msg) => {
        ...
        if (step == 1) {
          ...
          message.channel.send(
            "What should be the description of the application?"
          );
        } else if (step == 2) {
          application.description = msg.content;
          message.channel.send(
            "Where should be the responses sent in? Provide the ID of the channel"
          );
        } else if (step == 3) {
          if (!message.guild.channels.cache.get(msg.content))
            return collector.stop("INVALID_CHANNEL");
          application.responseChannelID = msg.content;
          message.channel.send(
            "Please specify the application status(Open or Closed)."
          );
        } else if (step == 4) {
          if (
            msg.content.toLowerCase() !== "open" &&
            msg.content.toLowerCase() !== "closed"
          )
            return;
          application.status = msg.content.toLowerCase();
          message.channel.send(
            "What questions do you want in the application? Say `done` when you have put all the questions"
          );
        } else if (step >= 5) {
          ...
          message.channel.send(
            `What is question ${application.questions.length + 1}?`
          );
        }
      });
      ...
      collector.on("end", async (msg, reason) => {
        if (reason == "INVALID_CHANNEL")
          return message.channel.send("The channel your provided is invalid");
        if (reason == "INVALID_NUMBER")
          return message.channel.send("The number is invalid");
      });
    }
    setup(message);
  },
