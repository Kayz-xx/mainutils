  ...
  async execute(client, message, cmd, args) {
    ...
    if (flags.get(` roles`)) {
      ...
      if (roles.length >= 1024) {
        return message.reply(`Roles cannot contain more than 1024 character`);
      }
    } else {
    ...
    rolesinfo.forEach(async (rolesinfo) => {
      let role = message.guild.roles.cache.find((r) => r.id === rolesinfo);
      if (!role)
      ...
      usersinfo.forEach(async (users) => {
        ...
        if (!user)
          ...
          if (user.roles.cache.some((r) => r.id === array[i].id)) return;
          await sleep(2500);
