...
module.exports = {
  ...
  async execute(client, message, cmd, args) {
    ...
    function heistStatistics(string, message) {
      ...
      let highestFine = Math.max.apply(
        null,
        usersFined.map((x) =>
          x.match(/\s\b\d[\d,.]*\b/g).map((e) => e.replace(/,/g, ""))
        )
      );

      let heistAmount = Number(
        message.match(/\W\b\d[\d,.]*\b/g)[0].replace(/`|,/g, "")
      );

      let heistValue = Number(
        usersHeisted[0].match(/\s\b\d[\d,.]*\b/g)[0].replace(/,/g, "")
      );
    ...
    let heistMessages = await message.channel.messages.fetch({
      after: messageId,
    });
    heistMessages = heistMessages.filter(
      (x) =>
        x.author.id === "270904126974590976" &&
        x.createdTimestamp > new Date().getTime() - 300000
    );
    ...
    collector.on("collect", (btn) => {
      if (btn.customId == "stats") {
        ...
        if (!found)
          embed.setDescription("Could not find your results in this heist.");
