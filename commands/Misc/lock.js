    ...
    try {
      ...
      if (
        ...
        return message.channel.send(
          `${channel} was already locked for \`${role.name}\``
        );
      ...
      message.channel.send(
        `<#${channel.id}> has been locked for \`${role.name}\``
      );
    } catch (error) {
      message.channel.send(
        "There was an error locking the channel, please make sure there are no errors in your message."
      );
    }
  },
};
