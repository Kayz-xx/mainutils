     ...
     if(cmd === 'fping'){
      ...
      if(cmd === 'mm'){
       ...
       if(message.channel.id !== "855628710706020362" && message.channel.id !== "764885370391232575") return;
        ...
        if(cmd === 'nitro'){
        if(message.guild.id === "764885367160700958") {
        if(message.channel.id !== "767829880930435133" && message.channel.id !== "764885369384599564") return;
        if(!message.member.roles.cache.some(x => x.id === '764885367400693764') && !message.member.permissions.has(Permissions.FLAGS.MANAGE_MESSAGES)) return;
        setTimeout(() => message.delete(), 100)
        const msg = args.slice(0).join(' ');
