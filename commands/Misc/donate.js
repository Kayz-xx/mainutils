...
async function donate(role, message, args, channelReq) {
  ...
  let embed = new MessageEmbed()
    ...
    .setTitle(
      `${message.author.tag} wants to donate ${prize} <a:im4:858370157890371595>`
    )
    .setThumbnail(
  ...
  const filter = (btn) =>
    message.guild.members.cache
      .find((member) => member.id === btn.user.id)
      .roles.cache.some((x) => x.id === role) ||
    message.guild.members.cache
      .find((member) => member.id === btn.user.id)
      .roles.cache.some((x) => x.id === "764885367400693764") ||
    message.guild.members.cache
      .find((member) => member.id === btn.user.id)
  ...
  collector.on("collect", async (btn) => {
    ...
    if (btn.customId === "del") {
      ...
      btn.channel.send(
        `<@${btn.user.id}> has **claimed** your donation! Make sure to send items/cash to them.`
      );
      btn.deferUpdate();
      msg.edit({
        embeds: [embed],
  ...
  collector.on("end", async (collected, reason) => {
    if (reason.toUpperCase() === "TIME") {
      ...
      msg.edit({
        content: `No one **claimed** the donation proceed **manually**.`,
        components: [rows2],
      });
...
module.exports = {
  name: "donate",
  aliases: ["donate", "eventdono", "heistdono", "wdonate", "edonate"],
  cooldown: "0",
  permissions: [],
  usage:
    "<prize> <time> <winners> <requirement> <message>, eventdono- <event> <winners> <prize> <req> <message>",
  ...
  async execute(client, message, cmd, args) {
    ...
    if (cmd === "eventdono") {
      ...
      if (!winners)
        return message.channel.send("Please specify an amount of winners");
      const prize = args[2];
      if (!prize) return message.channel.send("Please specify a prize");
      const requirement = args[3];
      if (!requirement)
        return message.channel.send("Please specify a requirement");
      ...
      let embed = new MessageEmbed()
        .setAuthor("Donations", message.author.avatarURL({ dynamic: true }))
        .setTitle(
          `${message.author.tag} wants to donate for : ${event} <a:im4:858370157890371595>`
        )
        .setThumbnail(
          "https://cdn.discordapp.com/icons/764885367160700958/a_38503e9dec18ac442fecaad24a3d07c0.gif?size=1024"
        )
      ...
      try {
        ...
        const filter = (btn) =>
          message.guild.members.cache
            .find((member) => member.id === btn.user.id)
            .roles.cache.some((x) => x.id === "792799102140022785") ||
          message.guild.members.cache
            .find((member) => member.id === btn.user.id)
            .roles.cache.some((x) => x.id === "764885367400693764") ||
          message.guild.members.cache
        ...
        collector.on("collect", async (btn) => {
          ...
          if (btn.customId === "del") {
            ...
            message.author.send({ embeds: [embed3] });
            btn.channel.send(
              `<@${btn.user.id}> has claimed your event donation! Make sure to send items to them.`
            );
            btn.deferUpdate();
            msg.edit({
              embeds: [embed],
          ...
          if (btn.customId === "nul") {
            ...
            btn.channel.send(
              `<@${btn.user.id}> has **declined** your donation.`
            );
            btn.deferUpdate();
            msg.edit({ components: [rows] });
            collector.stop();
        ...
        collector.on("end", async (collected, reason) => {
          if (reason.toUpperCase() === "TIME") {
            ...
            msg.edit({
              content: `No one **claimed** the event donation proceed **manually**.`,
              components: [rows2],
            });
    ...
    if (cmd === "heistdono") {
      ...
      let embed = new MessageEmbed()
        .setAuthor("Donations", message.author.avatarURL({ dynamic: true }))
        .setTitle(
          `${message.author.tag} wants to donate a ${prize} heist! <a:im4:858370157890371595>`
        )
        .setThumbnail(
          "https://cdn.discordapp.com/icons/764885367160700958/a_38503e9dec18ac442fecaad24a3d07c0.gif?size=1024"
        )
      ...
      try {
        ...
        const filter = (btn) =>
          message.guild.members.cache
            .find((member) => member.id === btn.user.id)
            .roles.cache.some((x) => x.id === "770365283147317248") ||
          message.guild.members.cache
            .find((member) => member.id === btn.user.id)
        ...
        collector.on("collect", async (btn) => {
          ...
          if (btn.customId === "del") {
            ...
            btn.user.send({ embeds: [embed2] });
            btn.channel.send(
              `<@${btn.user.id}> has **claimed** your heist donation! Make sure to send cash to them.`
            );
            btn.deferUpdate();
            msg.edit({
              embeds: [embed],
        ...
        collector.on("end", async (collected, reason) => {
          if (reason.toUpperCase() === "TIME") {
            ...
            msg.edit({
              content: `No one **claimed** the heist donation proceed **manually**.`,
              components: [rows2],
            });
