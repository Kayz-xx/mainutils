...
module.exports = {
	...
	async execute(client, message, cmd, args) {
		...
		if (cmd === 'snipe') {
			...
			if (snipes)
				snipes = snipes.filter((x) => x.type === 'Message Deleted');
			else
			...
			if (!target)
				return message.reply({
					content: `There is only ${snipes.length} messages!`,
				});
		...
		if (cmd === 'snipelist') {
			...
			let data = snipes.map((snipe, index) => {
				...
				if (type === 'Message Deleted')
					return `**${type} by ${msg.author.tag} (<t:${Math.floor(
						time / 1000,
					)}:R>)**\n${msg.content}`;
				else
					return `**${type} by ${msg.author.tag} (<t:${Math.floor(
		...
		if (cmd === 'esnipe') {
			let snipes = client.snipes.get(message.channel.id);
			if (snipes)
				snipes = snipes.filter((x) => x.type === 'Message Edited');
			else
				return message.reply({
					content: 'There is nothing to snipe in this channel',
				});
			...
			message.channel.send({
				embeds: [
					new MessageEmbed()
						...
						.setFooter(`${snipe + 1} / ${snipes.length}`)
