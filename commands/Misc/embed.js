      ...
      if (!message.member.permissions.has(Permissions.FLAGS.MANAGE_MESSAGES)) {
        ...
        let dataopt = {
         ...
         let slct = new MessageSelectMenu()
          ...
          .setDescription('Select **options** from the menu to create an embed.\n1. is the Image, 2. is the thumbnail')
          .setImage('https://cdn.discordapp.com/attachments/855455034187710476/881439617096773702/unknown.png')
          .setColor('FFFFFF')
        ...
        if(button.customId && button.customId === 'setDone'){
              button.reply({content: 'Please specify a channel id or mention, to send the embed to.', ephemeral: true})
              let filter = m => message.author.id === m.author.id
              let doneclr = button.channel.createMessageCollector({ filter, time: 30000 })
              
              doneclr.on("collect", async m => {
              const channel = message.guild.channels.cache.get(m.toString()) || m.mentions.channels.first()
              if(membed.content == "Preview") {
              channel.send({embeds: [membed.embeds[0] ] })
              membed.delete()
              e.delete()}
              else if(membed.content !== "Preview"){
              channel.send({ content: membed.content, embeds: [membed.embeds[0] ] })
              membed.delete()
              e.delete()
              }
              })
        ...
        if(/^#[0-9A-F]{6}$/i.test(m.content)) {
          ...
          titleclr.stop()
            ...
            if(button.values[0] === 'setURL'){
              ...
              const url = membed.embeds[0].image ? membed.embeds[0].image.url : '';
              let send = ''
              const l = m.content.includes("http")
