  ...
  async execute(client, message, cmd, args) {
    try {
      ...
      if (
        ...
        message.channel.id !== "774383258250248233"
      )
        return;
      ...
      if (state === true)
        return message.channel.send(
          `<#${channel.id}> has been viewunlocked for \`${role.name}\` with state \`${state}\``
        );
      else
        return message.channel.send(
          `<#${channel.id}> has been viewunlocked for \`${role.name}\``
        );
    } catch (error) {
      message.channel.send(
        "There was an error viewunlocking the channel, please make sure there are no errors."
      );
    }
  },
};
