...
module.exports = {
  ...
  async execute(client, message, cmd, args) {
    ...
    name = arr2[1];
    if (!name) return message.channel.send({ embeds: [embed2] });
    let amount = arr[1];
    if (!amount) return message.channel.send({ embeds: [embed3] });
    let aliases = arr[2];
    if (!aliases) return message.channel.send({ embeds: [embed4] });
    let type = arr[3];
    if (!type) return message.channel.send({ embeds: [embed5] });
    ...
    const found =
      ...
      data.find((item) => item.id === aliases);
    if (found) {
      let embed = new MessageEmbed()
