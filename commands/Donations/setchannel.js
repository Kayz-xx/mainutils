...
module.exports = {
    ...
    async execute(client, message, cmd,  args) {
        ...
        if (!channel)
            return message.channel.send({content:'I cannot find that channel. Please mention a channel within this server.'}).then(m => m.delete({timeout: 5000}));

         db.ref(`Donations/Info/${message.guild.id}/Settings/Channel`).set(channel.id)
