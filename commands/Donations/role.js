...
module.exports = {
  ...
  usage: "<role>",
  description:
    "Sets up autoroles(autoroles are roles automatically added when a user hits a specific amount of donation!)",
  category: "Donations",
  ...
  async execute(client, message, cmd, args) {
    try {
      ...
      message.channel.send({
        content:
          "This is the setup for autoroles, use the roles id's for setting them up ",
      });
      ...
      collector.on("end", (collected) => {
        ...
        collected.forEach((value) => {
          db.ref(
            `Donations/Info/${message.guild.id}/Settings/Donorole${index++}`
          ).set(value.content);
        });
      });
      ...
      if (data5.length === 0) {
        return message.reply({ content: "No Amount has been set" });
      }
    } catch (e) {
      console.log(e.stack);
