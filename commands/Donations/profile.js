...
const economy2 = require("../../functions/karuta");
const economy3 = require("../../functions/economy");
const { MessageEmbed } = require("discord.js");
const formatter = new Intl.NumberFormat("en");
const event = require("../../functions/eventdonations");
module.exports = {
  name: "profile",
  ...
  async execute(client, message, cmd, args) {
    ...
    if (cmd === "profile") {
      let embed = new MessageEmbed()
        ...
        .addField(
          `Owo Donation`,
          `${formatter.format(coins)} **Cowoncy** <:cowoncy:919665528811974667>`
        )
        .addField(
          `Karuta Donation`,
          `${formatter.format(coins2)} **Tickets** 🎟️`
        )
        .setTimestamp();
      if (che) {
        embed.setFooter(str);
    ...
    if (cmd === "eventprofile") {
      ...
      let embed = new MessageEmbed()
        .setAuthor(`${target.tag}'s 2yr Special Donations`)
        .setColor("RANDOM")
        ...
        .addField(
          ...
          `${formatter.format(
            coins5
          )} **Cowoncy** <:cowoncy:919665528811974667>`
        )
        .addField(
          `Karuta Donation`,
          `${formatter.format(coins4)} **Tickets** 🎟️`
        )
        .setTimestamp();
      if (che) {
        embed.setFooter(str);
