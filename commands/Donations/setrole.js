...
module.exports = {
    ...
    async execute(client, message, cmd,  args) {
      ...
      try{        if (!message.member.permissions.has(Permissions.FLAGS.MANAGE_GUILD))
        ...
        if (!role)
            return message.channel.send({content:'I cannot find that role, Please mention a role within this server.'}).then(m => m.delete({timeout: 5000}));
