...
module.exports = {
  ...
  async execute(client, message, cmd, args) {
    if (message.member.roles.cache.some((x) => x.id === "774008242127765535")) {
      ...
      if (parseInt(winners) > 20)
        return message.reply("The max amount of winners is 20");
      ...
      while ((m = regex.exec(string)) !== null) {
        ...
        if (m.groups?.Type === "role") {
          ...
          if (tem.similarity <= 0.6)
            return message.reply("No role found with the name or id provided");

          r = message.guild.roles.cache.find((r) => r.id == tem.item.id);
          if (!r)
            return message.reply({
              content: "This role could not be found in the server.",
            });
        ...
        if (m.groups?.Type === "bypass") {
          ...
          b = message.guild.roles.cache.find((r) => r.id == tem.item.id);
          if (!b)
            return message.reply({
              content: "This role could not be found in the server.",
            });
        ...
        if (m.groups?.Type === "blrole") {
          ...
          d = message.guild.roles.cache.find((r) => r.id == tem.item.id);
          if (!d)
            return message.reply({
              content: "This role could not be found in the server.",
            });
