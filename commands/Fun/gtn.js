...
module.exports = {
  ...
  async execute(client, message, cmd, args) {
    let number = args[0];
    if (!number)
      return message.reply({ content: "Cannot start without a number." });
    let times = args[1];
    ...
    embed.description = `You have **${convertTime(
      time
    )}** to guess the number, Good luck!`;
    ...
    if (publicGame) {
      ...
      const embed2 = new Discord.MessageEmbed()
        .setDescription(
          `${embed.description.replace(/{{time}}/g, convertTime(time))}`
        )
      ...
      gameCollector.on("collect", (button) => {
        if (button.user.id !== message.author.id) {
          return button.reply({
            content: othersMessage.replace(/{{author}}/g, message.author.id),
            ephemeral: true,
          });
        }
        button.deferUpdate();
        if (button.customId === id) {
          ...
          const _embed = new Discord.MessageEmbed()
            .setTitle(embed.title)
            .setDescription(loseMessage.replace(/{{number}}/g, number))
            .setColor(embed.color)
            .setFooter(embed.footer)
            .setTimestamp();
