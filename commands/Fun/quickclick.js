...
module.exports = {
  ...
  async execute(client, message, cmd, args) {
    ...
    startMessage = "First person to press the correct button will win!";
    ...
    Collector2.on("collect", async (_btn) => {
      if (_btn.member.id !== oppenent.id) {
        return _btn.reply({
          content: othersMessage.replace("{{author}}", `<@${oppenent.id}>`),
          ephemeral: true,
        });
      }
      ...
      if (_btn.customId === "deny") {
        ...
        const emd = new Discord.MessageEmbed()
          .setTitle(embed.title)
          .setDescription(`Did not accept the challenge.`)
          .setFooter(embed.footer)
          .setColor(embed.color);
        if (embed.timestamp) {
          emd.setTimestamp();
        ...
        setTimeout(async function () {
          ...
          const _embed = new Discord.MessageEmbed()
            ...
            .setFooter(embed.footer)
            .setDescription(
              startMessage.replace("{{time}}", convertTime(time))
            );
          if (embed.timestamp) {
            _embed.setTimestamp();
          }
          ...
          const Collector = msg.createMessageComponentCollector({
            filter: (btn) =>
              btn.user.id === oppenent.id || btn.user.id === challenger.id,
            time: time,
          });
