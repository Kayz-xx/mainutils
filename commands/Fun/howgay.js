...
module.exports = {
  ...
  async execute(client, message, cmd, args) {
    ...
    if (!op.some((el) => type.toLowerCase().includes(el)))
      return message.reply("Valid Options: `High, Low`");
    ...
    let denybutton = new MessageButton()
      ...
      .setLabel("Deny")
      .setCustomId("deny");
    let row = new MessageActionRow().addComponents([acceptbutton, denybutton]);
    const embed = new MessageEmbed()
      .setTitle("Howgay Fight")
      .setDescription(
        `<@${message.author.id}> challenged <@${member.id}> to a fight, do you accept?`
      )
      .setFooter("Good Luck")
      .setTimestamp()
      .setColor("RANDOM");
    ...
    Collector.on("collect", async (btn) => {
      if (btn.member.id !== member.id) {
        return btn.reply({
          content: "This is not your choice.",
          ephemeral: true,
        });
      }
      btn.deferUpdate();
    ...
    Collector.on("end", async (collected, reason) => {
      ...
      let rows2 = new MessageActionRow().addComponents(
        question.components[0].components
      );
