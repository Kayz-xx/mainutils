...
module.exports.run = async (client, giveaway, member, reaction) => {
  ...
  if (
    !role.every((r) => member.roles.cache.has(r)) &&
    !bypass.some((r) => member.roles.cache.has(r)) &&
    !ad.some((r) => member.roles.cache.has(r))
  ) {
    ...
    let embed = new MessageEmbed()
      ...
      .setColor("FF3F00")
      .setDescription(
        `You must have this role to participate in the giveaway: \`${nam.replace(
          "undefined",
          ""
    ...
    member
      .send({ embeds: [embed] })
      .catch(() => console.log("Could not send user DM!"));
  } else if (b == false) {
    if (
      !role.every((r) => member.roles.cache.has(r)) &&
      !ad.some((r) => member.roles.cache.has(r))
